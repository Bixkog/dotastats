...
fn extract_match_stats(member_names: &Vec<MemberName>, match_json: serde_json::Value) -> Result<Match> {
    if match_json["players"].is_null() {return Err(serde_json::Error::custom("No players data."))}
    let match_players : Vec<serde_json::Value> = match_json["players"].as_array().unwrap().clone().to_vec();
...
fn compute_parsing_stats(matches : &Vec<Match>) -> HashMap<String, u32> {
    ...
    fn update_field (field_cnt: &mut HashMap<String, u32>, k : String, v : serde_json::Value) {
        match v {
            ...
            serde_json::Value::Array(arr) => for e in arr {
    ...
    for match_ in matches.iter() {
        update_field(&mut field_cnt, String::new() , serde_json::to_value(match_).unwrap());
    };
    field_cnt
}