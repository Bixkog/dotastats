...
impl ParseRequester {
    ...
    async fn check_replay_availability(
        &self,
        match_json: &serde_json::Value,
    ) -> reqwest::Result<bool> {
        let start_inst = Instant::now();
        let request_url = if match_json["replay_url"].is_null() {
            if match_json["cluster"].is_null()
                || match_json["match_id"].is_null()
                || match_json["replay_salt"].is_null()
            {
                return Ok(false);
            }
            format!(
                "http://replay{}.valve.net/570/{}_{}.dem.bz2",
                match_json["cluster"].as_u64().unwrap(),
                match_json["match_id"].as_u64().unwrap(),
                match_json["replay_salt"].as_u64().unwrap()
            )
        } else {
            match_json["replay_url"].as_str().unwrap().to_string()
        };
    ...
    async fn send_parse_request(&self, match_id: &MatchId) -> reqwest::Result<()> {
        let start_inst = Instant::now();
        reqwest::Client::new()
            .post(&format!("https://api.opendota.com/api/request/{}", match_id).to_string())
    ...
    async fn get_parse_state(
        ...
        if replay_present {
            ...
            return Ok(ParseRequestState {
                match_id,
                state: ParseState::Sent,
            });
        } else {
            println!("Match is not parsed. Replay unavailable.");
            return Ok(ParseRequestState {
                match_id,
                state: ParseState::ReplayMissing,
            });
        }
    }

    /// For each match with unknown parsing state, gets its parse state and saves it in storage.
    pub async fn request_parsing(
        &self,
        guild_id: &GuildId,
        match_jsons: &Vec<serde_json::Value>,
    ) -> reqwest::Result<()> {
        let old_parse_states: HashSet<MatchId> = self
            .storage
            .get_parse_states(guild_id)
            .into_iter()
