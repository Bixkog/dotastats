...
impl MatchStorage {
    ...
    pub fn get_info<'a>(
        &self,
        guild_id: &GuildId,
        match_ids: &HashSet<MatchId>,
    ) -> (Vec<MatchId>, Vec<serde_json::Value>) {
        ...
        match utils::read_lines(&self.ids_filename(guild_id)) {
            ...
            Ok(cached_ids) => {
                let not_cached_ids: Vec<MatchId> = match_ids
                    .difference(
                        &cached_ids
                            .into_iter()
    ...
    pub fn get_match_data(&self, guild_id: &GuildId) -> Vec<serde_json::Value> {
        let match_data = utils::read_lines(&self.info_filename(guild_id)).unwrap();
        match_data
            .iter()
            .map(|s| serde_json::from_str(s).unwrap())
            .collect()
    ...
    pub fn add_info(&self, guild_id: &GuildId, match_infos: &Vec<serde_json::Value>) {
        let match_ids: Vec<String> = match_infos
            .iter()
            .map(|v| v["match_id"].as_u64().unwrap().to_string())
            .collect();
        let raw_match_infos = match_infos.iter().map(|v| v.to_string()).collect();
        utils::append_lines(&self.ids_filename(guild_id), &match_ids)
            .expect("Unable to write to guild_ids.");
        utils::append_lines(&self.info_filename(guild_id), &raw_match_infos)
            .expect("Unable to write to guild_info.");
    }
    ...
    pub fn get_parse_states(&self, guild_id: &GuildId) -> Vec<ParseRequestState> {
        let requests =
            utils::read_lines(&self.req_filename(guild_id)).expect("Unable to read requests.");
        requests
            .iter()
            .map(|s| serde_json::from_str(s).unwrap())
            .collect()
    ...
    pub fn update_parse_states(&self, guild_id: &GuildId, request_states: &Vec<ParseRequestState>) {
        utils::clear_file(&self.req_filename(guild_id)).expect("Can't clear req file.");
        self.add_parse_states(guild_id, &request_states);
    }
}
